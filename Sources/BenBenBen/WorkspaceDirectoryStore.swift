...
final class WorkspaceDirectoryStore: ObservableObject {
    ...
    private static let markdownWorkingDirectoryKey = "benbenben.markdownWorkingDirectory"
    private static let shellWorkingDirectoryKey = "benbenben.shellWorkingDirectory"
    private static let pythonProjectDirectoryKey = "benbenben.pythonProjectDirectory"
    private static let appleScriptDirectoryKey = "benbenben.appleScriptDirectory"
    private static let launchdDirectoryKey = "benbenben.launchdPath"
    private static let benshellRootDirectoryKey = "benbenben.benshellRootDirectory"
    private static let condaRootDirectoryKey = "benbenben.condaRootDirectory"
    private static let legacyMarkdownWorkingDirectoryKey = "notchwow.markdownWorkingDirectory"
    private static let legacyShellWorkingDirectoryKey = "notchwow.shellWorkingDirectory"
    private static let legacyPythonProjectDirectoryKey = "notchwow.pythonProjectDirectory"
    private static let legacyAppleScriptDirectoryKey = "notchwow.appleScriptDirectory"
    private static let legacyLaunchdDirectoryKey = "notchwow.launchdPath"

    init() {
        markdownWorkingDirectory = AppDefaults.string(
            forKey: Self.markdownWorkingDirectoryKey,
            migrating: [Self.legacyMarkdownWorkingDirectoryKey, "notchNotes.markdownWorkingDirectory"]
        )
            ?? WorkspacePaths.markdownRoot.path
        let savedShellWorkingDirectory = AppDefaults.string(
            forKey: Self.shellWorkingDirectoryKey,
            migrating: [Self.legacyShellWorkingDirectoryKey, "notchNotes.shellWorkingDirectory"]
        )
        ...
        pythonProjectDirectory = AppDefaults.string(
            forKey: Self.pythonProjectDirectoryKey,
            migrating: [Self.legacyPythonProjectDirectoryKey, "notchNotes.pythonProjectDirectory"]
        )
            ?? WorkspacePaths.pythonRoot.path
        appleScriptDirectory = AppDefaults.string(
            forKey: Self.appleScriptDirectoryKey,
            migrating: [Self.legacyAppleScriptDirectoryKey, "notchNotes.appleScriptDirectory"]
        )
            ?? WorkspacePaths.appleScriptRoot.path
        launchdDirectory = AppDefaults.string(
            forKey: Self.launchdDirectoryKey,
            migrating: [Self.legacyLaunchdDirectoryKey, "notchNotes.launchdPath"]
        )
            ?? WorkspacePaths.launchdRoot.path
        let savedBenshellRoot = AppDefaults.string(
            forKey: Self.benshellRootDirectoryKey,
            migrating: ["notchwow.benshellRootDirectory"]
        )
        let obsoleteBenshellRoots = Set([
            WorkspacePaths.legacyBenshellRoot.standardizedFileURL.path,
            WorkspacePaths.integratedBenshellRoot.standardizedFileURL.path,
            "/Users/ben/Desktop/notchwow/Scripts/benshell"
        ])
        if savedBenshellRoot.map(Self.normalizedPath).map(obsoleteBenshellRoots.contains) == true {
        ...
        condaRootDirectory = AppDefaults.string(
            forKey: Self.condaRootDirectoryKey,
            migrating: ["notchwow.condaRootDirectory"]
        )
