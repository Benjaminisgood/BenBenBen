...
`BenBenBen` 是 Ben 的个人 Codex 工作台，也是一个常驻 MacBook 刘海的原生 macOS 伙伴。主窗口负责 Agent 会话、个人知识、Scripts、Python 与 `launchd` Jobs；“Ben龙”入口负责快速唤起、状态提醒和轻量交互。

原始资料始终保存在 `~/keyoti`。App 的 SQLite 数据库只保存可重建的搜索索引和 UI 元数据，不替代 Markdown、脚本、Python、AppleScript 或 plist 文件。

## 核心能力

- 原生主窗口：使用 `NavigationSplitView`、Inspector 与 macOS 26 Liquid Glass，集中查看 Home、Agents、Knowledge、Tools 和 Automations。
- Codex 入口：通过外部 `codex app-server --stdio` 管理 ChatGPT 登录、线程、流式事件、Diff、审批和中断；不复制 Codex 的 `auth.json`。
- 个人知识与任务：索引 `~/keyoti` 中的 Markdown、Shell、Python、AppleScript 和 Jobs；识别 Markdown checkbox、`TODO:`、`待完成:`、日期与生活/学习/工作标签。
- 现有工作台：保留 MarkdownEngine、文件锁、Conda/Python、Terminal.app 启动和 `launchd` 适配器。
- Ben龙刘海伙伴：关闭主窗口后仍可驻留；无刘海屏幕回退为顶部浮动入口。
...
“登录时启动”默认关闭。启用后由签名在主 bundle 内的 Login Item helper 仅启动 Ben龙伙伴，不自动打开主工作台。
